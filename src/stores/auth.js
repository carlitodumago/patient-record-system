...
export const useAuthStore = defineStore("auth", () => {
  ...
  const login = async (credentials) => {
    ...
    try {
      ...
      if (!isEmail(email)) {
        try {
          // Use RPC function to bypass RLS - the Users table blocks anon access,
          // but we need to look up the username before the user is authenticated
          ...
          // RPC returns an array of rows; check if we got a match
          const userRecord = Array.isArray(rpcResult)
            ? rpcResult[0]
            : rpcResult;
          ...
          foundUserId = userRecord.user_id;
        } catch (usernameError) {
