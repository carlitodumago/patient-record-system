...
const historyItems = computed(() => {
  // Get all visits
  const allVisits = visits.value.map((visit) => ({
    ...visit,
    type: "Visit",
    ...
    time:
      visit.visitTime ||
      (visit.timestamp
        ? new Date(visit.timestamp).toTimeString().slice(0, 5)
        : "09:00"),
  ...
  const allRecords = records.value.map((record) => ({
    ...record,
    type: "Record",
    ...
    time:
      record.recordTime ||
      (record.timestamp
        ? new Date(record.timestamp).toTimeString().slice(0, 5)
        : "09:00"),
  ...
  return [...allVisits, ...allRecords].sort((a, b) => {
              ...
              <tr v-for="item in historyItems" :key="`${item.type}-${item.id}`">
                ...
                <td>
                  ...
                  <router-link
                    :to="
                      item.type === 'Visit'
                        ? `/patients/${item.patientId}?tab=visits`
                        : `/records?record=${item.id}`
                    "
