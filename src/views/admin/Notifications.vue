          ...
          <v-card-text>
            ...
            >
              ...
              <template v-slot:item.actions="{ item }">
                ...
                <v-btn icon small color="error" @click="deleteNotification(item)">
    ...
    <v-dialog v-model="showAddDialog" max-width="600px">
      <v-card>
        ...
        <v-card-text>
          <v-form ref="form" v-model="valid">
            <v-row>
              ...
              <v-col cols="12" v-if="notificationForm.recipientType === 'specific'">
                <v-select
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" @click="sendNotification" :loading="sending" :disabled="!valid">
            Send
          </v-btn>
        </v-card-actions>
      </v-card>
      ...
      <v-card v-if="selectedNotification">
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="12">
              <strong>Recipient:</strong> {{ selectedNotification.recipientName || 'All Users' }}
            </v-col>
            <v-col cols="12">
              <strong>Created:</strong> {{ formatDate(selectedNotification.createdAt) }}
            </v-col>
            <v-col cols="12">
              <strong>Read:</strong> {{ selectedNotification.read ? 'Yes' : 'No' }}
            </v-col>
          </v-row>
        </v-card-text>
        <v-card-actions>
...
const loadNotifications = async () => {
  ...
  try {
    ...
    notifications.value = data.map(item => ({
      ...item,
      recipientName: item.users ? `${item.users.firstName} ${item.users.lastName}` : 'All Users'
    }))
  } catch (error) {
...
const sendNotification = async () => {
  ...
  try {
    ...
    if (notificationForm.value.recipientType === 'all') {
      ...
      notificationsToInsert.push(...users.map(user => ({
...
const confirmDelete = async () => {
  ...
  try {
    const { error } = await supabase
      .from('notifications')
      .delete()
      .eq('id', selectedNotification.value.id)
