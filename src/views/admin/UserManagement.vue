              ...
              <table class="table table-hover">
                ...
                <tbody>
                  <tr>
                    ...
                    <td>admin@patientrecord.system</td>
...
const users = ref([])
const roles = ref([
  { id: 'admin', name: 'Administrator', description: 'Full system access', color: 'danger' },
  { id: 'nurse', name: 'Nurse/Staff', description: 'Patient management access', color: 'info' },
  { id: 'patient', name: 'Patient', description: 'Personal records access', color: 'secondary' }
])
...
const validateUserForm = (userData, errors) => {
  ...
  // Check for duplicate email/username (excluding current user in edit mode)
...
const addUser = async () => {
  ...
  if (!newUser.value.password?.trim()) {
    formErrors.value = { ...formErrors.value, password: 'Password is required' }
  } else if (newUser.value.password.length < 6) {
    formErrors.value = { ...formErrors.value, password: 'Password must be at least 6 characters' }
  }
