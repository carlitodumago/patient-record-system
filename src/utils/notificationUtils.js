...
export const addVisitNotification = (notificationStore, visit, getPatientName, action, userStore) => {
  ...
  const physicianInfo = visit.physicianName ? `with ${visit.physicianName}` : '';
  
  switch (action) {
    case 'scheduled':
      title = 'New Visit Scheduled';
      message = `A visit for ${patientName} has been scheduled for ${dateTimeString} ${physicianInfo}`.trim() + '.';
      type = 'info';
      break;
    case 'completed':
      title = 'Visit Completed';
      message = `Visit for ${patientName} on ${dateTimeString} ${physicianInfo} has been marked as completed.`;
      type = 'success';
      break;
    case 'updated':
      title = 'Visit Updated';
...
export const showToast = (message, options = {}) => {
  ...
  try {
    ...
    import('../composables/useNotify').then(module => {
